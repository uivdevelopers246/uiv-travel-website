...
| Variable                                          | Purpose                                                     |
| ------------------------------------------------- | ----------------------------------------------------------- |
| `APP_ENV`                                         | Notification delivery environment: `local`, `staging`, or `production`. Set explicitly in deployed environments. |
| `NOTIFICATION_TRANSPORT`                          | Explicit delivery transport: `resend` or `webhook`. |
| `NOTIFICATION_DELIVERY_ENABLED`                   | Delivery kill switch. `false` leaves notification events queued without contacting the provider. |
| `NOTIFICATION_RECIPIENT_POLICY`                   | `allowlist` or `unrestricted`. Staging requires `allowlist`. |
| `NOTIFICATION_EMAIL_ALLOWLIST`                    | Comma-separated staging test recipients. Required and non-empty when `APP_ENV=staging`. |
...
| `STRIPE_SECRET_KEY`                               | **Server-only.** Stripe API (`getStripe()` in `@/lib/stripe/server`): Checkout Session, refunds. Restricted keys need at least **Checkout Sessions (write)** and **Charges and Refunds (write)**. See [Booking & checkout (M4)](#booking--checkout-m4-high-level) and `docs/adrs/ADR-M4-B-shopping-cart-and-checkout.md`. |
...
| `CRON_SECRET`                                     | **Server-only.** Shared secret for cron routes. The queue worker is `POST /api/cron/notifications`; digest generation is `GET /api/cron/notification-digest`; callers send `Authorization: Bearer <CRON_SECRET>`. |
| `RESEND_API_KEY`                                  | **Server-only.** Resend API key used by the M6 notification worker to send transactional and digest email from Vercel Functions. |
| `EMAIL_FROM`                                      | **Server-only.** Verified sender address passed to Resend for notification email. |
| `RESEND_WEBHOOK_SECRET`                           | **Server-only.** Resend/Svix webhook signing secret for `POST /api/webhooks/resend/events`; used to verify raw webhook bodies before recording delivery events. |
| `EMAIL_DELIVERY_WEBHOOK_URL` / `EMAIL_DELIVERY_WEBHOOK_SECRET` | **Server-only, optional.** Provider-neutral transport credentials, used only when `NOTIFICATION_TRANSPORT=webhook`. |

## Booking & checkout (M4) (High-level)

Paid **activity** and **accommodation** bookings share cart → Checkout **setup** (save payment method) → vendor approval per line → **one** off-session settlement charge. Carts may be activity-only, stay-only, or **mixed**. Stripe env vars above apply to this flow; operational detail for webhooks and cron is under [Background Jobs / Webhooks](#background-jobs--webhooks).

- **ADRs (source of truth for design):**
...
- **Operations:**
  - [`docs/m4-activity-operations-runbook.md`](m4-activity-operations-runbook.md) — MVP incident and manual follow-up (activities + stays)
  - [`docs/m4-activity-money-manual-test.md`](m4-activity-money-manual-test.md) — money-flow test checklist (activities, stays, mixed)
- **Booking ledger inserts (DB):**
  - **`activity_bookings`** and **`accommodation_bookings`** rows are created only through **privileged server paths**: verified Stripe setup webhooks (**service role**) via `create_activity_booking_after_payment` / `create_accommodation_booking_after_setup` — not via the publishable key.
  - The **`authenticated`** role has **no** `INSERT` policy on either booking table.
- **Checkout gate:**
  - `validateCartForCheckout` in `lib/cart/service.ts` accepts activity, accommodation, or mixed carts; re-prices and re-validates each line kind before Stripe setup.
- **M4-C after Checkout setup (`mode: setup`):**
  - Order → **`awaiting_vendor_approval`**; activity and stay lines → **`pending_approval`** until vendors approve or decline (or SLA expiry / admin decline).
  - Setup fulfillment creates both booking kinds from cart lines; clears the cart only when all line kinds succeed; compensates **both** tables on partial failure (`lib/stripe/server.ts`).
  - When **every** line on **both** tables is terminal and **at least one** is **`confirmed`**, `lib/orders/settlement.ts` **`tryBeginSettlementChargeForOrder`** creates **one** off-session **`PaymentIntent`** for the sum of confirmed line totals across both tables (`metadata.flow = m4c_settlement`).
- **Settlement ordering (create vs attach vs webhook):**
  - Creation uses `confirm: true`, so Stripe may emit **`payment_intent.succeeded`** before the app attaches the new PI id to `orders.stripe_payment_intent_id`.
  - Fulfillment allows transition from **`awaiting_vendor_approval`** / **`payment_pending`**; settlement start must not cancel a succeeded or already order-bound PI when an attach race is detected (see `lib/orders/settlement.ts` and tests).
- **Settlement success (`payment_intent.succeeded`):**
  - Handled by **`fulfillSettlementPaymentIntentSucceeded`** in `lib/stripe/server.ts` → **`updateOrderPaidAfterSettlementCapture`** when captured amount and currency match **`computeConfirmedSettlementTotalCents`** (confirmed activity **and** stay bookings) and the order currency.
  - Order → **`paid`**.
- **Settlement amount or currency mismatch:**
  - **`markOrderReconciliationRequiredAfterSettlementMismatch`** in `lib/orders/service.ts` moves the order to **`reconciliation_required`** (when the PI id still matches `orders.stripe_payment_intent_id` and status is **`awaiting_vendor_approval`** or **`payment_pending`**).
  - Treated as **non-retryable**: event recorded in **`stripe_webhook_events`**, fulfillment result **`reconciliation_required`**, **`POST /api/webhooks/stripe`** returns **200** so Stripe does not retry; ops compares Dashboard vs `orders` / `activity_bookings` / `accommodation_bookings` (optional audit rows in **`order_settlement_mismatches`**).
- **Booking HTTP APIs (cookie client, RLS):**
  - Consumer: `src/app/api/activity-bookings` (read, cancel)
  - Vendor: `src/app/api/vendor/activity-bookings` (list/detail); `src/app/api/vendor/accommodation-bookings/[id]/{approve,decline}`
  - Admin: `src/app/api/admin/activity-bookings` (list; PATCH to **`completed`**); `src/app/api/admin/accommodation-bookings/[id]/{approve,decline}`
  - Handlers use `lib/activity-bookings/service.ts` / `lib/accommodation-bookings/service.ts` (and `lib/orders/vendor-approval.ts` for approve/decline) with `createClient()` from `@/lib/supabase/server`.
...
- **Supabase Postgres:** Main schema in `public`. Tables: `profiles` (1:1 with `auth.users`), `vendors` (owner + optional business profile columns), `site_admins` (user_id → auth.users), `activities` and **`accommodations`** (both `vendor_id → vendors`), plus M4 **`orders`**, **`cart_lines`**, **`availability_slots`**, **`activity_bookings`**, and **`accommodation_bookings`** for paid activity and stay checkout. Activities have text `location` (display) and optional `location_point`. Accommodations have address/parish fields and optional `location_point`. Location geometry is set only via RPCs, not direct column writes from the app.
- **RLS:** Enabled on `profiles`, `vendors`, `activities`, `accommodations`, `site_admins`. Policies mirror the activities model for accommodations (published public read; vendor/admin for own rows). **Vendors:** owners can read/insert/update their row (subject to column grants); admins retain elevated policies from consolidated migrations. Column-level grants on `activities` and `accommodations` limit inserts/updates (e.g. no `rating` on activities; no `location_point` or `is_featured` on accommodations for authenticated direct writes). **M4:** `activity_bookings` and `accommodation_bookings` allow **select** for owners, vendors, and admins; **`authenticated` has no insert policy** on either — webhook fulfillment uses **service role** (see ADRs).
- **RPCs:** `set_activity_location_point` and `set_accommodation_location_point` set or clear PostGIS points (`st_setsrid(st_makepoint(...), 4326)`). Granted to `authenticated` only; underlying UPDATE still enforced by RLS. **M4 activities:** `create_activity_booking_after_payment` (atomic slot lock + capacity check + insert, `SECURITY DEFINER`, **service_role** only), `cancel_activity_booking` (user cancel). **M4 stays (ADR-M4-D):** `create_accommodation_booking_after_setup`, `cancel_accommodation_bookings_for_order`, `expire_pending_accommodation_bookings`, confirm/decline RPCs, `accommodation_stay_is_held` (soft overlap for cart/checkout). Details in `docs/database.md`.
...
- **M4-C / M4-D SLA expiry and settlement:** `GET /api/cron/pending-approval-expiry` requires **`Authorization: Bearer $CRON_SECRET`** and uses **`createServiceRoleClient()`** to call **`expire_pending_activity_bookings`** and **`expire_pending_accommodation_bookings`**. Each RPC expires rows and returns **`expired_count`** plus distinct **`order_ids`** from the same database transaction (no separate client-side list query vs **`now()`**). For each distinct returned order, the cron then runs the same **`syncOrderM4cAfterBookingChange`** post-transition hook used by vendor approve/decline. The hook calls **`syncOrderDeclinedWhenNoPendingHoldsRemain`** (declines an order when all activity **and** stay bookings are non-confirmed terminal) and **`tryBeginSettlementChargeForOrder`** (starts one settlement when at least one booking is confirmed across either table and all remaining bookings are terminal). Confirmed settlement totals sum **`total_cents`** from both **`activity_bookings`** and **`accommodation_bookings`**. Scheduled in **`vercel.json`** (e.g. every 15 minutes).
- **M6 notifications:** notification hooks write `notification_events` rows and attempt transactional delivery immediately through the explicitly configured Resend or webhook transport. `POST /api/cron/notifications` is the authenticated queue worker; `GET /api/cron/notification-digest` is the authenticated daily digest generator. Workers use leased PostgreSQL claims, retry transient failures, and preserve the notification dedupe key as the provider idempotency key. `POST /api/webhooks/resend/events` verifies the Svix signature, stores idempotent sent/delivered/delayed/failed/bounced/complained/suppressed feedback, and suppresses future email for permanent recipient outcomes. See `docs/notifications.md` for setup and operations.
- **M4-C / M4-D admin bulk decline:** **`declineActivityOrderAsAdmin`** in **`lib/orders/vendor-approval.ts`** declines pending rows on **both** booking tables, then uses the same **`syncOrderM4cAfterBookingChange`** hook as vendor approve/decline paths (not a direct **`orders.status = declined`** update), so **`syncOrderDeclinedWhenNoPendingHoldsRemain`** and **`tryBeginSettlementChargeForOrder`** apply consistently when an order still has **confirmed** lines after pendings are declined. Per-line stay approve/decline APIs live under **`/api/vendor/accommodation-bookings/[id]/{approve,decline}`** and **`/api/admin/accommodation-bookings/[id]/{approve,decline}`**.
- **Stripe:** `POST /api/webhooks/stripe` verifies `Stripe-Signature` with `STRIPE_WEBHOOK_SECRET`, then uses **`createServiceRoleClient()`** for DB work (orders, pending `activity_bookings` and `accommodation_bookings` after Checkout **setup**, cart clearing, idempotency in `stripe_webhook_events`). **M4-C setup:** handle **`checkout.session.completed`** (setup mode) and **`setup_intent.succeeded`** so the saved payment method and order state are recorded. **M4-C settlement:** handle **`payment_intent.succeeded`** and **`payment_intent.payment_failed`** for PaymentIntents whose **`metadata.flow`** is **`m4c_settlement`** (see `fulfillSettlementPaymentIntentSucceeded` / `fulfillSettlementPaymentIntentPaymentFailed` in `lib/stripe/server.ts`); success marks the order **`paid`** when totals align; **amount/currency mismatch** marks **`reconciliation_required`** and still **acks (200)** after recording **`stripe_webhook_events`**; failure may schedule **one** retry PI then terminal **`failed`**. Stripe retries on non-2xx responses.
  ...
  - **Manual follow-up expectations for acknowledged non-retryables:**
    ...
    - Investigate and remediate in ops playbooks (for example: compare Stripe Dashboard objects vs `orders` / `activity_bookings` / `accommodation_bookings`, then refund, correct data, or move order state as appropriate).
...
- **Decision: `activity_bookings` / `accommodation_bookings` inserts are webhook / service-role only at the DB**
  - Why: Enforces “payment method saved before pending hold” for normal users; prevents authenticated clients from inserting reservation rows without going through Stripe setup fulfillment.
  - Implications: Verified `/api/webhooks/stripe` uses a **service-role** Supabase client and `create_activity_booking_after_payment` / `create_accommodation_booking_after_setup` (not user JWT inserts). User cancellation of confirmed activities uses `POST /api/activity-bookings/[id]/cancel` → `cancel_activity_booking` RPC. Confirmed stays use the twin **`cancel_accommodation_booking`** RPC (buyer HTTP cancel route may still be frontend-follow-on). See ADR-M4-A / ADR-M4-B / ADR-M4-D and `docs/database.md`.
...
- **Decision: M4-C single settlement `PaymentIntent` per order (ADR-M4-C / ADR-M4-D)**
  - Why: Checkout collects a payment method only; vendors approve or decline per booking line (activity **or** stay); the customer is charged **once** for the **sum of confirmed** lines after every line on **both** booking tables is terminal, avoiding per-vendor PaymentIntents.
  - Implications: `orders.stripe_payment_intent_id` holds the active settlement PI; `settlement_charge_attempt_count` distinguishes first charge vs one retry; `metadata.flow = m4c_settlement` on the PI ties webhooks to this path; `lib/orders/settlement.ts`, `lib/orders/order-booking-lines.ts`, and `lib/stripe/server.ts` implement creation and fulfillment across mixed carts.
- **Decision: Settlement capture mismatch → `reconciliation_required` + webhook 200**
  ...
  - Implications: Explicit terminal-ish order state for support; **`stripe_webhook_events`** insert preserves idempotency; route maps `reconciliation_required` fulfillment result to **200**; manual reconciliation compares Stripe, `orders`, `activity_bookings`, and `accommodation_bookings` (optional use of **`order_settlement_mismatches`** from migrations for structured audit rows).
