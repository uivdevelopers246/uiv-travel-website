...
- **M4-C SLA expiry (no Stripe):** `GET /api/cron/pending-approval-expiry` requires **`Authorization: Bearer $CRON_SECRET`** and uses **`createServiceRoleClient()`** to run **`expire_pending_activity_bookings`** then **`syncOrderDeclinedWhenNoPendingHoldsRemain`** per affected order (sets **`orders.status = declined`** when no **`pending_approval`** remains and every booking is **`declined` / `expired` / `cancelled`**). Scheduled in **`vercel.json`** (e.g. every 15 minutes); does not create settlement **`PaymentIntent`**s.
- **Stripe:** `POST /api/webhooks/stripe` verifies `Stripe-Signature` with `STRIPE_WEBHOOK_SECRET`, then uses **`createServiceRoleClient()`** for DB work (orders, pending `activity_bookings` after Checkout **setup**, cart clearing, idempotency in `stripe_webhook_events`). **M4-C:** subscribe to **`checkout.session.completed`** (setup mode) and **`setup_intent.succeeded`**; settlement adds **`payment_intent.succeeded`** / **`payment_intent.payment_failed`** when implemented. Stripe retries on non-2xx responses.
