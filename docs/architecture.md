...
Paid activity bookings use cart → Checkout **setup** (save payment method) → vendor approval per line → **one** off-session settlement charge. Stripe env vars above apply to this flow; operational detail for webhooks and cron is under [Background Jobs / Webhooks](#background-jobs--webhooks).
...
- **`activity_bookings` inserts (DB):**
  - Rows are created only through **privileged server paths**: verified Stripe setup webhooks (**service role**) or site admin — not via the publishable key.
  - The **`authenticated`** role has **no** `INSERT` policy on `activity_bookings`.
- **M4-C after Checkout setup (`mode: setup`):**
  - Order → **`awaiting_vendor_approval`**; lines → **`pending_approval`** until vendors approve or decline (or SLA expiry / admin decline).
  - When **every** line is terminal and **at least one** is **`confirmed`**, `lib/orders/settlement.ts` **`tryBeginSettlementChargeForOrder`** creates **one** off-session **`PaymentIntent`** for the sum of confirmed line totals (`metadata.flow = m4c_settlement`).
...
- **Settlement success (`payment_intent.succeeded`):**
  - Handled by **`fulfillSettlementPaymentIntentSucceeded`** in `lib/stripe/server.ts` → **`updateOrderPaidAfterSettlementCapture`** when captured amount and currency match **`computeConfirmedSettlementTotalCents`** (confirmed bookings) and the order currency.
  - Order → **`paid`**.
- **Settlement amount or currency mismatch:**
  ...
  - Treated as **non-retryable**: event recorded in **`stripe_webhook_events`**, fulfillment result **`reconciliation_required`**, **`POST /api/webhooks/stripe`** returns **200** so Stripe does not retry; ops compares Dashboard vs `orders` / `activity_bookings` (optional audit rows in **`order_settlement_mismatches`**).
- **Booking HTTP APIs (cookie client, RLS):**
  - Consumer: `src/app/api/activity-bookings` (read, cancel)
  - Vendor: `src/app/api/vendor/activity-bookings` (list/detail)
  - Admin: `src/app/api/admin/activity-bookings` (list; PATCH to **`completed`**)
  - Handlers use `lib/activity-bookings/service.ts` with `createClient()` from `@/lib/supabase/server`.
