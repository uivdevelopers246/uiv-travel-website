...
| `CRON_SECRET`                                     | **Server-only.** Shared secret for **`GET /api/cron/pending-approval-expiry`** and **`GET /api/cron/notifications`**: caller must send **`Authorization: Bearer <CRON_SECRET>`**. On Vercel, set in Project → Environment Variables; Vercel Cron includes this header automatically when the variable is defined. |
...
| `EMAIL_DELIVERY_WEBHOOK_URL` / `EMAIL_DELIVERY_WEBHOOK_SECRET` | **Server-only, optional.** Provider-neutral fallback email delivery webhook used only when Resend is not configured. |
...
- **M6 notifications:** notification hooks write `notification_events` rows and process transactional email immediately through Resend (`lib/notifications/email-worker.ts` + `lib/notifications/resend.ts`) or the optional provider-neutral delivery webhook fallback. `GET /api/cron/notifications` requires **`Authorization: Bearer $CRON_SECRET`**, creates provider daily digest outbox rows, and processes queued email. `POST /api/webhooks/resend/events` accepts Resend webhooks, verifies the Svix signature with `RESEND_WEBHOOK_SECRET`, stores delivery/bounce/complaint/reject events in `email_delivery_events`, and suppresses future email on bounce or complaint.
