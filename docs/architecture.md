...
- **M4-C SLA expiry and settlement:** `GET /api/cron/pending-approval-expiry` requires **`Authorization: Bearer $CRON_SECRET`** and uses **`createServiceRoleClient()`** to call **`expire_pending_activity_bookings`** once. That RPC both expires rows and returns **`expired_count`** plus distinct **`order_ids`** from the same database transaction (no separate client-side list query vs **`now()`**). For each returned order, the cron then runs the same **`syncOrderM4cAfterBookingChange`** post-transition hook used by vendor approve/decline. The hook calls **`syncOrderDeclinedWhenNoPendingHoldsRemain`** (declines an order when all bookings are non-confirmed terminal) and **`tryBeginSettlementChargeForOrder`** (starts one settlement when at least one booking is confirmed and all remaining bookings are terminal). Scheduled in **`vercel.json`** (e.g. every 15 minutes).
