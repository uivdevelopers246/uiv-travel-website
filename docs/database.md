...
### M4: `availability_slots`, `activity_bookings`, `accommodation_bookings`, `orders`, `cart_lines`, `stripe_webhook_events`
...
| **`accommodation_bookings`** | Stay ledger (ADR-M4-D). Columns: listing/user/vendor/order FKs; **`check_in`** / **`check_out`** (`date`, half-open `[check_in, check_out)`); **`guests`**; money snapshots; full M4-C status set + **`expires_at`**. **Select:** booking owner, vendor owner, site admin. **`INSERT`:** not for **`authenticated`** — service role via **`create_accommodation_booking_after_setup`** (lock listing `FOR UPDATE`, reject overlapping confirmed / non-expired pending holds, insert). **User cancel:** `cancel_accommodation_booking` RPC. No GiST exclusion / `btree_gist` in this milestone. |
...
| **`cart_lines`** | Per-user cart lines; activity lines use `slot_id` → `availability_slots` (no `activity_id` on row). Accommodation lines use **`accommodation_id`**, **`check_in`**, **`check_out`**, **`guests`** (activity columns null, and vice versa). Price fields are snapshots. **Partial unique** on `(user_id, slot_id)` where `line_type = 'activity'`; **partial unique** on `(user_id, accommodation_id, check_in, check_out)` where `line_type = 'accommodation'`. **RLS** (`20260403120800_rls_cart_lines.sql`): **`authenticated`** **FOR ALL** on own rows (`user_id = auth.uid()`). |
...
| **`accommodation_bookings`** | **authenticated:** own rows; vendor owner; site admin. | (No authenticated INSERT; service role / admin `FOR ALL`.) | Admin `FOR ALL`; buyers cancel via RPC only. | Admin `FOR ALL`. |
...
| **accommodation_stay_is_held** | Soft overlap: true if a **confirmed** or non-expired **pending_approval** stay overlaps `[p_check_in, p_check_out)`. | `p_accommodation_id`, `p_check_in`, `p_check_out` | — | **`anon`**, **`authenticated`**, **`service_role`** | `SECURITY DEFINER` (accurate soft holds under RLS). Twin of slot capacity snapshot for cart/public. |
| **create_accommodation_booking_after_setup** | Lock listing `FOR UPDATE`, validate published + vendor + guests vs **`max_guest_capacity`**, reject overlapping holds, insert `accommodation_bookings`. | Accommodation, user, vendor, order ids; dates; guests; cent columns; optional `p_discount_cents` (default 0), **`p_status`** (default **`pending_approval`**), **`p_expires_at`** (required when pending). | See `20260726120100_create_accommodation_booking_rpcs.sql`. | **`service_role` only** | `SECURITY DEFINER`; half-open overlap; no GiST exclusion. |
| **cancel_accommodation_booking** | User sets own **`confirmed`** stay to **`cancelled`**. | `p_booking_id` | — | **`authenticated`**, **`service_role`** | `SECURITY DEFINER`. |
| **cancel_accommodation_bookings_for_order** | Fulfillment rollback: **`confirmed`** \| **`pending_approval`** → **`cancelled`**. | `p_order_id` | — | **`service_role` only** | Clears **`expires_at`**. |
| **expire_pending_accommodation_bookings** | SLA sweep for stays. | none | — | **`service_role` only** | Returns `jsonb` `{ expired_count, order_ids }` like activities. Also called by **`GET /api/cron/pending-approval-expiry`** (alongside the activity expiry RPC). |
| **confirm_pending_accommodation_booking_for_vendor** / **`_as_admin`** | **`pending_approval` → `confirmed`** for one stay. | booking id (+ vendor id for vendor variant) | — | **`service_role` only** | Per-booking approve (ADR-M4-D). |
| **decline_pending_accommodation_booking_for_vendor** / **`_as_admin`** | **`pending_approval` → `declined`** for one stay. | booking id (+ vendor id for vendor variant) | — | **`service_role` only** | Per-booking decline. |
| **decline_pending_accommodation_bookings_for_vendor_on_order** | Vendor-scoped bulk decline on an order. | `p_order_id`, `p_vendor_id` | — | **`service_role` only** | Returns rows updated. |
| **decline_accommodation_bookings_for_order** | All pending stays on order → **`declined`**. | `p_order_id` | — | **`service_role` only** | Prefer vendor-scoped when multi-vendor. |
...
| cart_lines_user_accommodation_stay_key | cart_lines | user_id, accommodation_id, check_in, check_out (partial unique, `line_type = 'accommodation'`) | One merged stay line per listing + dates (ADR-M4-D). |
| idx_accommodation_bookings_accommodation_confirmed | accommodation_bookings | accommodation_id (partial, `status = 'confirmed'`) | Confirmed stay inventory. |
| idx_accommodation_bookings_accommodation_pending_approval | accommodation_bookings | accommodation_id, expires_at (partial pending) | Pending hold lookups. |
| idx_accommodation_bookings_pending_approval_expires_at | accommodation_bookings | expires_at (partial pending) | SLA expiry sweep. |
...
32. `20260423120000_m4c_reconciliation_required_status_and_mismatch_audit.sql` — reconciliation status / mismatch audit (see file).
33. `20260427121000_expire_pending_activity_bookings_returns_jsonb.sql` — **`expire_pending_activity_bookings`** returns `jsonb` `{ expired_count, order_ids }`.
34. `20260726120000_create_accommodation_bookings.sql` — **`accommodation_bookings`** table, indexes, `updated_at` trigger (ADR-M4-D).
35. `20260726120100_create_accommodation_booking_rpcs.sql` — stay soft-hold, create-after-setup (lock + overlap), cancel/expire/confirm/decline RPCs.
36. `20260726120200_rls_accommodation_bookings.sql` — RLS on **`accommodation_bookings`**; **`cancel_accommodation_booking`** RPC (no authenticated INSERT).
37. `20260726120300_cart_lines_accommodation_checks.sql` — accommodation CHECKs, activity null-other-type CHECK tighten, stay merge unique index.
