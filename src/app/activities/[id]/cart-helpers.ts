...
export function getAddToCartSuccessMessage(input: {
  ...
  if (input.mergedParticipants > input.requestedParticipants) {
    return `Updated your cart to ${formatParticipantsLabel(input.mergedParticipants)} for ${input.slotDateTimeLabel}. Review your cart to save your payment method.`;
  }

  return `Added ${formatParticipantsLabel(input.requestedParticipants)} for ${input.slotDateTimeLabel}. Review your cart to save your payment method and submit the request.`;
}
