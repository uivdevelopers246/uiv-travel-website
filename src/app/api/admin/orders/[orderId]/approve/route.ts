...
import { approveActivityOrderAsAdmin } from "@/lib/orders/vendor-approval";
import {
  parseUuidParam,
  requireRole,
  serverError,
...
export async function POST(
  ...
  try {
    const result = await approveActivityOrderAsAdmin(parsed.id);
    ...
    if (
      ...
      message.startsWith("No pending approval bookings")
    ) {
